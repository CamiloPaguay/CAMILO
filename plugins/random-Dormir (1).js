const handler = async (m, { conn, text, command }) => {
  try {
    const yh = global.lolix;

    if (!Array.isArray(yh) || yh.length === 0) {
      throw new Error("La lista de imágenes no está disponible.");
    }
    ...
    await conn.sendMessage(m.chat, { image: { url: url }, caption: mensaje }, { quoted: m });
  } catch (error) {
    console.error("Error al enviar el mensaje:", error);
    await conn.sendMessage(m.chat, "Lo siento, ocurrió un error al intentar enviarte una imagen. Por favor, inténtalo de nuevo más tarde.", { quoted: m });
  }
};
